...
Return ONLY the enhanced bullet point, nothing else. Do not include quotes or any additional text.`;

  try {
    console.log(`Enhancing bullet for ${role} at ${company}:`, bulletPoint.substring(0, 50));
    ...
    if (!response.ok) {
      ...
      if (errorData.error?.code === 429) {
        console.error('⚠️ Gemini API quota exceeded. Daily limit: 20 requests');
...
export async function enhanceExperience(experiences) {
  ...
  const enhanced = await Promise.all(
    experiences.map(async (exp) => {
      ...
      // Split description into bullet points (only by newline, not periods)
...
export async function enhanceProfileForPDF(profile, options = {}) {
  ...
  if (!enhanceContent) {
    console.log('Content enhancement disabled');
    return profile;
  }

  if (!GEMINI_API_KEY) {
    console.warn('⚠️ GEMINI_API_KEY not set - skipping AI enhancement');
    console.warn('Get a free API key from: https://makersuite.google.com/app/apikey');
    console.warn('Add to .env.local: GEMINI_API_KEY=your_key_here');
    return profile;
  }

  console.log('🤖 Enhancing profile content with AI...');
